...
### 使用引导：你需要准备什么、AI 会问你什么

想一次顺利到底，**说这句话之前，先准备 3 样**：

1. **文件**——CSV 或 xlsx 的路径。文件在哪、叫什么，最好直接说出来。
2. **表头在哪**（可选）——第 1 行就是列名？还是第 1 行是大标题、第 2 行才是列名？不知道就说"帮我判断"，AI 会自动扫。
3. **想要什么风格**（可选）——如水蓝 / 深海蓝 / 墨玉绿 / 珊瑚橙… 拿不准就说"你看着办"，默认是经典水蓝。

> 这 3 样**都不是必须的**。你只说一句"把这表弄好看点"，AI 也会自动推断并兜底，选项只影响你想要的差异化效果。
...
AI：好的。我看到文件了，大致是「部门 / 销量 / 单价 / 金额」这样的结构。
   ① 表头在第 1 行对吗？还是前面有大标题？
你：第 1 行就是表头。

AI：② 想用哪套配色？
   经典商务：水蓝 / 深海蓝 / 墨玉绿 / 陨石灰蓝
   暖色高级：勃艮第红 / 珊瑚橙
   青春活力：樱花粉 / 暖阳橙 / 薰衣草紫 / 抹茶绿 / 蜜桃 / 雾蓝紫
你：水蓝就行。

AI：③ 文件里色阶是"越高越绿"，要不要也统一成像水蓝主题？
   建议保留（默认），或改成主题色、跳过。
你：保留吧。

AI：好的，开始生成…
   ✅ 已完成：D:\报表\销售月度_美化.xlsx
```

**几点省心的规则**：

- **哪里不清楚就回"你定"**——AI 会自动推断表头、用默认水蓝配色，不阻塞。
- **想改随时说**——生成后不满意，直接说"把表头改成第 2 行"或"换珊瑚橙"，AI 重新跑，不丢数据。
- **强调表头**——表头是最影响效果的一项，说清楚"第几行是列名"能省去纠错环节。
