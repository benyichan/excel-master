    ...
    try:
        ...
        if pd.api.types.is_numeric_dtype(clean):
            ...
            if pd.api.types.is_float_dtype(sample):
                # 0~1 之间的浮点 → 百分比（占比、完成率等）
                # 注意：金额也可能是 0~1 的小数（如单价），所以加 nunique > 2 排除只有单一值的字段
...
def _beautify_worksheet(ws, col_types_override: Optional[dict] = None, theme: str = 'default',
    ...
    for c in range(start_col, end_col + 1):
        ...
        if ct in ('money', 'number', 'pct'):
            for r in range(data_start, data_end + 1):
                ...
                if isinstance(v, str) and v.startswith('=') and len(v) > 15:
                    est = 15
                    break
        ws.column_dimensions[col_letter].width = est
    ...
    for c in range(start_col, end_col + 1):
        ...
        for r in range(data_start, data_end + 1):
            ...
            if is_num:
                ...
                if is_formula:
                    cell.font = s['data_font_black']  # 公式=黑
                else:
    ...
    # 6. 边框（规则5）
    # border_start：如果 A 列有数据，边框从 A 列开始；否则从 start_col（B列）开始
    data_rows = data_end - header_row + 1
